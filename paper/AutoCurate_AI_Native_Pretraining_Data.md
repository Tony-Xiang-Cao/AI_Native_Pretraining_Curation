...
3. Verifier-gated self-improvement (Section 6): a hill-climber that adopts a
   ...
   over-flagging, the data-curation instance of reward overoptimization — and
   show the guard delivers a hard no-false-positive guarantee that an un-guarded
   balanced objective, though empirically safer, cannot [1].
...
Gopher's — symbol-to-word ratio, bullet/ellipsis line fractions, a stop-word
check, mean word length, duplicate line/paragraph fractions — and C4's
hand-built rules [2,3]. RefinedWeb and FineWeb codify extraction (trafilatura)
plus Gopher-style filtering plus dedup [4,5]; DCLM establishes curation as a
benchmarkable design space and finds model-based filtering the biggest lever
[6]. Our profiler (Section 3) reuses these signals, adds a compression-ratio
screen, and — the key difference — never freezes them: they are the *evolving
artifact*.

Compression as a quality signal. Compression ability tracks model capability
near-linearly [7]. The "entropy law" links corpus compression ratio to training
loss and motivates low-redundancy selection [8]. ZIP-FIT selects data by
gzip-distance alone [9]. We use gzip ratio as a cheap two-sided
redundancy/garble screen.

Reference-free OCR quality. The dictionary/garbage ratio is the workhorse
no-reference signal [10]. "Rerunning OCR" predicts OCR accuracy from output-only
features such as n-gram statistics and garbage-token detection [11]. A survey of
OCR evaluation tools and metrics maps which metrics need ground truth [12]. Our
OCR gate (Section 4) is a transparent member of this family.

HTML main-content extraction and its evaluation. Trafilatura is the extractor
under RefinedWeb/FineWeb [13]. The standard way to *evaluate* boilerplate
removal is precision/recall/F1 against gold main text, where Trafilatura's mean
F1 ≈ 0.94 [14]. Our HTML gate estimates extraction quality *reference-free* — no
gold main text — and the mutation oracle supplies the labels the gold benchmark
otherwise would.

Self-improvement and verifiable rewards. STaR keeps only verifiably-correct
self-generated rationales [15]. RLVR replaces a learned reward with a
deterministic verifier [16]. FunSearch and AlphaEvolve evolve programs under an
automated evaluator [17,18]. This *accept-rule* machinery — propose, verify,
keep-if-better — has been aimed at reasoning and code, never at the *curation
policy itself*; we adopt the verifier-gated accept rule, not the open-ended
program-discovery capability (our search is a small parameter hill-climb,
Section 6). The cautionary result that defines our rule is that optimizing hard
against a proxy reward makes the gold reward decline — reward overoptimization /
Goodhart [1]. Hence improvement must be scored on a verifier over held-out
documents and held-out corruption vocabulary, disjoint from what is optimized.

Data validation and drift. TFDV and Great Expectations encode schema
inference, anomaly detection, and drift checks for production ML [19]; we adopt
their spirit for the JSON gate and the SPC monitor.
...
verifier, and is explicit that this is the only safe way to let a curator
improve itself [1].
...
Each of the four governance directions maps to a concrete module/artifact, as
summarized in Table 1.

Table 1. The four governance directions and the concrete `autocurate` module and
artifact (equation → code) that operationalizes each.
...
The governance parameters `Θ` are collected in one dataclass, `AutoCurateConfig`:
per-gate thresholds, cascade band, robust-z cutoff, SPC limits, and hill-climb
budget. One `tick()` of the loop (Figure 1) is a single
Observe→Orient→Decide→Act pass over a day's shards.
...
The per-gate defect-detection scores from the mutation oracle are reported in
Table 2 and visualized in Figure 2.

Table 2. Defect-detection F1 (5 corruption seeds, 80 clean docs/modality, on
...
Figure 2. Held-out floor vs. train upper-bound defect-detection F1 per gate
(E1), with the clean-guard false-positive rate.
...
Table 3. Quality vs. judge-call budget (E2). `e = 0` is heuristic-router-only;
...
Figure 3. Cascade quality (macro-F1) as a function of the fraction of documents
escalated to the judge (E2); the curve is monotone up to the operating point.

As shown in Table 3 and Figure 3, at half the judge calls the cascade recovers
~69% of the judge-everything
...
The end-state of each regime is reported in Table 4 and Figure 4.

Table 4. End-state after self-improvement (held-out documents), E3.
...
Figure 4. Held-out F1 and clean-guard false-positive rate for the verified vs.
naive hill-climbing regimes (E3); the un-guarded recall objective reward-hacks.
...
The aggregate drift response is summarized in Table 5, with a representative
stream shown in Figure 5.

Table 5. Drift response (mean over 8 streams), E4.
...
Figure 5. A representative 30-day stream (E4): mean extraction quality with the
EWMA/CUSUM control limits, the day-15 parser-regression shift, and the alarms.
...
Anomaly-detection F1 across cutoffs is reported in Table 6 and plotted in
Figure 6.

Table 6. Anomaly-detection F1 across cutoffs (E5; headline at z = 3.5: robust
...
Figure 6. Anomaly-detection F1 vs. cutoff z for the robust (median/MAD) detector
against the non-robust mean ± kσ detector (E5).
...
Detection F1 before and after the drift-triggered self-improvement episode is
reported in Table 7 and Figure 7.

Table 7. Held-out F1 at detecting the newly-drifting tag-leak defect (E6).
...
Figure 7. Held-out detection F1 on the newly-appearing tag-leak defect before
vs. after the verified hill-climb, against the naive-recall control (E6).
...
Table 8 and Figure 8 compare each gate against the best external reference-free
baseline for its modality.

Table 8. Held-out defect-detection F1 (5 seeds), E7: AutoCurate gate vs. the
best external reference-free baseline per modality.
...
Figure 8. AutoCurate gate vs. the best external reference-free baseline per
modality on the same held-out mutation oracle (E7).
...
2. Rae, J. et al. *Scaling Language Models: Methods, Analysis & Insights from Training Gopher.* 2021. arXiv:2112.11446.
3. Raffel, C. et al. *Exploring the Limits of Transfer Learning with a Unified Text-to-Text Transformer (T5/C4).* JMLR, 2020. arXiv:1910.10683.
4. Penedo, G. et al. *The RefinedWeb Dataset for Falcon LLM.* NeurIPS Datasets & Benchmarks, 2023. arXiv:2306.01116.
5. Penedo, G. et al. *The FineWeb Datasets: Decanting the Web for the Finest Text Data at Scale.* NeurIPS Datasets & Benchmarks, 2024. arXiv:2406.17557.
6. Li, J. et al. *DataComp-LM: In Search of the Next Generation of Training Sets for Language Models.* NeurIPS, 2024. arXiv:2406.11794.
7. Huang, Y. et al. *Compression Represents Intelligence Linearly.* COLM, 2024. arXiv:2404.09937.
8. Yin, M. et al. *Entropy Law: The Story Behind Data Compression and LLM Performance.* NeurIPS, 2024. arXiv:2407.06645.
9. Obbad, E. et al. *ZIP-FIT: Embedding-Free Data Selection via Compression-Based Alignment.* 2024. arXiv:2410.18194.
10. Alex, B. & Burns, J. *Estimating and Rating the Quality of Optically Character Recognised Text.* DATeCH, 2014.
11. Schneider, P. & Maurer, Y. *Rerunning OCR: A Machine Learning Approach to Quality Assessment and Enhancement Prediction.* Journal of Data Mining & Digital Humanities, 2021. arXiv:2110.01661.
12. Neudecker, C. et al. *A Survey of OCR Evaluation Tools and Metrics.* HIP@ICDAR, 2021.
13. Barbaresi, A. *Trafilatura: A Web Scraping Library and Command-Line Tool for Text Discovery and Extraction.* ACL-IJCNLP System Demos, 2021.
14. Bevendorff, J. et al. *An Empirical Comparison of Web Content Extraction Algorithms.* SIGIR, 2023.
15. Zelikman, E. et al. *STaR: Bootstrapping Reasoning With Reasoning.* NeurIPS, 2022. arXiv:2203.14465.
16. Lambert, N. et al. *Tülu 3: Pushing Frontiers in Open Language Model Post-Training (RLVR).* 2024. arXiv:2411.15124.
...
19. Caveness, E. et al. *TensorFlow Data Validation: Data Analysis and Validation in Continuous ML Pipelines.* SIGMOD, 2020.
